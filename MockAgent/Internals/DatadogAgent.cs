...
namespace MockAgent
{
    public class DatadogAgent : IDisposable
    {
        ...
        public DatadogAgent(WindowsPipesConfig config)
        {
            throw new NotImplementedException("Windows named pipes are not yet implemented in the MockAgent");
        }

        public DatadogAgent(int port = 8126, int retries = 5, bool useStatsd = false, bool doNotBindPorts = false, int? requestedStatsDPort = null)
        {
            _cancellationTokenSource = new CancellationTokenSource();
            ...
            while (true)
            {
                ...
                var containerHostname = Environment.GetEnvironmentVariable("CONTAINER_HOSTNAME");
        ...
        /// Can be different from <see cref="MockAgent(int, int)"/>'s <c>initialPort</c>
