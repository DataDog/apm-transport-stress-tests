...
func main() {
	...
	statsdClient.Incr("transport_sample.load.run", nil, 1)
	...
	for {
		...
		case <-sigs:
			...
			statsdClient.Count("transport_sample.load.span_created", spanDiff, nil, 1)

			fmt.Printf("Finishing at: %v\n", time.Now().Unix())
			fmt.Printf("Spans created: %f\n", spansCreated)
	        statsdClient.Count("transport_sample.load.span_logged", spansCreated, nil, 1)
	        statsdClient.Incr("transport_sample.load.end", nil, 1)
